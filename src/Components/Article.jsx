...
const Article = () => {
  ...
  const renderSection = (section) => (
    <div className="mb-4" key={section.section_title}>
      <h2 className="text-[15px] text-gray-500 lg:text-xl font-bold py-4">
        <FontAwesomeIcon icon={faDiamond} className="text-cyan-900 pr-2" />
        {section.section_title}
      </h2>
      {section.section_content && (
        <p className="text-[13px] lg:text-[15px]">{section.section_content}</p>
      )}
  ...
  const renderFAQSection = (faqSection) => {
  ...
  const renderContent = () => {
    ...
    const sections = articleData.sections.filter(
    ...
    return (
      <>
        {content}
        {faqSection && renderFAQSection(faqSection)}
      </>
    );
  };
